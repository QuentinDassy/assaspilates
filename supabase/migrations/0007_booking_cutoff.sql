-- Bookings close 1h before the class starts -- until now nothing stopped a
-- client from booking a course that had already started (the front-end only
-- filtered whole past *days*, so this morning's 09:00 class stayed bookable
-- all afternoon), and neither PHP caller nor the RPC checked the time.
--
-- Enforced here rather than in PHP because both booking paths
-- (site/api/create-payment-intent.php and site/api/book-with-carnet.php) go
-- through this one function, same as the TEACHER_ABSENT rule.
--
-- Times are compared in Europe/Paris: course_date + start_time is a wall-clock
-- timestamp with no zone, and the studio is in Paris, so it's anchored there
-- instead of inheriting whatever the database session timezone happens to be.

-- Full re-declaration (same pattern as 0004/0005) with one new check inserted
-- after the teacher-absence check.
